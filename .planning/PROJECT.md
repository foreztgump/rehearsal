...
**Goal:** Swap the voice pipeline to user-selectable Ollama LLMs and NVIDIA Nemotron streaming ASR with VRAM-aware STT placement, make `docker compose up` on the user's own machine the sole supported deployment (consumer-GPU detection/passthrough via the NVIDIA Container Toolkit, no VM path), add an optional client-side 3D avatar, and finish the deferred v1.0 polish — without regressing sub-1s voice-to-voice latency or the persona content guardrail.
...
- [ ] `docker compose up` on the user's own machine with consumer-GPU detection/passthrough as the sole supported deployment (no VM path)
...
- **v1.1 — Deployment:** `docker compose up` on the user's own machine is the SOLE supported deployment. GPU detection and passthrough go through the NVIDIA Container Toolkit (`--gpus all` / the existing `deploy.resources.reservations.devices` blocks) on the host running Compose — there is no VM/PCIe-passthrough path. The 16GB target GPU budget and local-first privacy posture are unchanged.
- **v1.1 — Zero-cert localhost install (2026-06-26):** the v1.0 TLS reverse proxy (Caddy + mkcert LAN cert on `:443`/`:7443`) was removed from the default stack. It existed ONLY to give the mic (`navigator.mediaDevices`) a secure context over `https://<lan-ip>` — but `localhost` is ALREADY a secure context, so the mic + WebRTC work over plain `http://localhost` with no certificates. Default install is now `cp .env.example .env && docker compose up`, then open `http://localhost:3000` (browser talks to `web:3000` + `ws://localhost:7880` directly). Stack dropped from seven services to six (no `proxy`). The mkcert/Caddy path is retained as OPTIONAL, documented for serving OTHER LAN devices over `https://<lan-ip>` (`certs/README.md`, `proxy/Caddyfile`, README "Serving other LAN devices"). Chromium is the recommended browser for a local install; Firefox drops loopback (`127.0.0.1`) ICE candidates by default (`media.peerconnection.ice.loopback=false`).
...
| **v1.1** `docker compose up` on the user's own machine is the SOLE deployment (NVIDIA Container Toolkit GPU passthrough; no VM/Proxmox path) | Lower friction; consumer-GPU target, single deployment story | — Pending (v1.1 deployment) |
| **v1.1** Remove the TLS reverse proxy from the default stack; localhost install is zero-cert | `localhost` is already a secure context, so the mic/WebRTC need no TLS; the Caddy+mkcert chain was pure friction for a local install | ✓ 2026-06-26 — `proxy` service removed (7→6 services), browser → `http://localhost:3000` + `ws://localhost:7880`; mkcert/Caddy kept OPTIONAL for LAN-device serving |
...
---
*Last updated: 2026-06-26 — v1.1 in progress; logged zero-cert localhost install (TLS proxy removed, optional for LAN).*
