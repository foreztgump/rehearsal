...
**IMPORTANT FINDING — Check A's `ollama show --template` assertion was OBSOLETE for gemma4 on Ollama 0.30 → NOW FIXED.** All three gemma4 tags (INCLUDING the official stock `gemma4:e2b`) report a bare `{{ .Prompt }}` passthrough from `ollama show --template` and have NO visible `<start_of_turn>`/`<end_of_turn>` markers — yet multi-turn role tracking works perfectly live (a 3-turn + system-prompt conversation correctly recalled "Bob"). Ollama 0.30 applies the gemma4 chat template **internally** (engine-side jinja/built-in renderer; runner launches with `--chat-template chatml --no-jinja`), so the markers no longer surface through `show --template`. The old structural scrape would now false-FAIL every gemma4 build, including stock.

**FIX (committed):** `ollama/verify-build.sh` Check A was rewritten from a `show --template` scrape to a **behavioral role-tracking probe** — a deterministic 3-turn `/v1` conversation (system + user-states-a-fact + assistant-ack + user-asks-it-back) that must recall the distinctive earlier-turn token `ZEBRA-7`. Broken role-turn rendering → failed recall → FAIL. Verified: **all 3 tags PASS** (stock `gemma4:e2b`, Fast, Better); negative controls FAIL correctly (a non-recall answer and a nonexistent/non-loadable tag both fail cleanly, with a hardened JSON parser that reports the `/v1` error instead of a traceback). Check B (the think=false artifact scan) was already valid and remains the load-bearing safety gate.
