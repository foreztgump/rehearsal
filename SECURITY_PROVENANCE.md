...
| Vendored avatar GLBs | 3D faces committed under `web/public/avatars/`; per-file source + non-commercial license recorded in `web/public/avatars/ATTRIBUTION.md`, and TalkingHead compatibility (rig/visemes/blendshapes, no meshopt) is enforced by `scripts/verify-avatars.mjs`. | Add per-file sha256 checksums before a public release, and pin the upstream example-set commit the faces were sourced from. |
